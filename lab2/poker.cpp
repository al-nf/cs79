...
namespace csen79 {
	...
	bool Poker::isFlush(uint s[]) { 
        for (int i = 0; i < Card::Suit::CLUB + 1; ++i) {
            if (s[i] >= 5) { return true; }
        }
        return false; 
    }
	bool Poker::isQuad(uint r[]) { 
        if (Poker::countRank(r, 4)) { return true; };
        return false; 
    }
	bool Poker::isTriple(uint r[]) { 
        if (Poker::countRank(r, 3)) { return true; };
        return false; 
    }
	bool Poker::isPair(uint r[]) { 
        if (Poker::countRank(r, 2)) { return true; };
        return false;
    }
	bool Poker::is2Pair(uint r[]) { 
        int pair_counter = 0;
        for (int i = 0; i < Card::NRANKS; ++i) {
            if (r[i] >= 2) { pair_counter++; }
        }
        return pair_counter == 2 ? true : false;
    }
